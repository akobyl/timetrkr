      ...
      <div class="card-header">
        ...
        >
          ...
          <h3 class="mb-3 mb-lg-0">Time Analysis</h3>
           <!-- Desktop layout -->
          <div
            class="d-none d-lg-flex date-controls-wrapper align-items-center"
          >
...
@click.prevent="selectedPreset = ''; closeDropdown('desktop')"
            ...
            <div
              class="desktop-date-inputs d-flex"
              v-show="!selectedPreset || selectedPreset === ''"
            >
...
@click.prevent="selectedPreset = ''; closeDropdown('mobile')"
        ...
        <div
          class="d-block d-lg-none date-controls-custom mt-2"
          v-show="!selectedPreset || selectedPreset === ''"
        >
        ...
        <div class="selected-date-range mt-2" v-if="startDate && endDate">
           <small class="text-muted"
            > Viewing data from {{ formatDateDisplay(startDate) }} to {{
      ...
      <div class="card-body">
        ...
        <div
          v-else-if="dailyHours.length === 0"
          class="text-muted text-center py-5"
        >
           <i class="bi bi-bar-chart-line" style="font-size: 3rem"></i> <br /><br />
          No time entries found for the selected date range. <br /> <button
            class="btn btn-outline-primary mt-3"
            @click="setDefaultDateRange"
          >
             Reset to Default Range </button
          >
        </div>
...
const fetchTimeData = async () => {
  ...
  try {
    ...
    if (
      dateRange.length > 0 &&
      dateRange[dateRange.length - 1] !== endDate.value
    ) {
      console.warn(
        `Warning: Date range does not include the end date! End date: ${endDate.value}, Last date in range: ${dateRange[dateRange.length - 1]}`
      )
    }
    ...
    console.log(
      `Original start date: ${startDateObj.toISOString()} (day of week: ${startDateObj.getDay()})`
    )
    ...
    console.log(
      'Starting week calculation with first Sunday:',
    ...
    while (weekStart <= endDateObj) {
      ...
      if (weekStart < startDateObj) {
        console.log(
          `Skipping week that starts before our range: ${weekStart.toISOString()}`
        )
        // Move to next week
      ...
      const displayStartStr = weekStart.toLocaleDateString(undefined, {
        month: 'short',
        day: 'numeric',
      })
      const displayEndStr = weekEnd.toLocaleDateString(undefined, {
        month: 'short',
        day: 'numeric',
      })

      console.log(
        `Week ${weekIndex}: ${weekStartStr} (${displayStartStr}) to ${weekEndStr} (${displayEndStr})`
      )
    ...
    dailyHours.value.forEach((day) => {
      ...
      if (!found) {
        console.warn(
          `Day ${day.rawDate} doesn't match any week in range ${startDate.value} to ${endDate.value}`
        )
      }
    })
    ...
    entries.forEach((entry) => {
      ...
      const startBlock = Math.floor((startHour * 60 + startMin) / 5)
...
const getWeekDates = (date) => {
  ...
  console.log(
    `Week dates for ${d.toISOString()}: ${start.toISOString()} to ${end.toISOString()}`
  )
...
const generateDateRange = (start, end) => {
  ...
  if (dates.length > 0) {
    console.log(
      `Generated ${dates.length} dates from ${dates[0]} to ${dates[dates.length - 1]}`
    )
  } else {
